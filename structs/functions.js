...
function UpdateTokens() {
    fs.writeFileSync("./token/tokens.json", JSON.stringify({
...
function updateLvlAndXp(currentLevel, currentXp, xpEarned) {
    ...
    if (!levelsFilePath) {
        throw new Error("Failed to read level data.");
    }
    ...
    for (let i = newLevel - 1; i < levelsData.length; i++) {
        ...
        if (!xpNeeded || newLevel >= 100) break;
...
function getBPLvl(level, levelEarned) {
    const bpFilePath = path.join(__dirname, `../responses/Athena/XP/bp.json`);
    const bpList = JSON.parse(fs.readFileSync(bpFilePath, 'utf8'));

    let totalBPEarned = 0;
    let totalLevel = level + levelEarned;

    for (var i = level + 1; i <= 100; i++) {
        if (totalLevel < bpList[i].level) break;
        totalBPEarned += bpList[i].bpReward;
    }
...
function updateBPLvl(bpLevel, bpLevelEarned) {
    const totalBpLevel = bpLevel + bpLevelEarned;
    const passLevels = Math.floor(totalBpLevel / 10);
    const remainingBpLevel = totalBpLevel % 10;

    return {
        passLevel : passLevels,
        bpLevel : remainingBpLevel
    };
}

async function updateUserLevel(req, user) {
    try {
        const findProfiles = await Profile.findOne({ accountId: user.accountId });
        const levelByXP = JSON.parse(fs.readFileSync(path.join(__dirname, "../responses/Athena/XP/xp.json")).toString());
        ...
        let leveledUp = false;
        const nextLevelInfo = levelByXP.find(l => l.level === attributes.level + 1);

        if (!nextLevelInfo) {
            return { success : false, message: "Max level reached or Level info for next level not found.." };
        }

        if (attributes.xp >= nextLevelInfo.xpToNextLvl) {
            leveledUp = true;
            attributes.level = nextLevelInfo.level;
            const xpIncrements = {
            ...
            if (xpIncrements[attributes.level]) {
                attributes.book_xp += xpIncrements[attributes.level];
            } else {
                attributes.book_xp += 2;
            }

            const newLevelInfo = levelByXP.find(l => l.level === attributes.level);
            attributes.xp = attributes.xp - nextLevelInfo.xpToNextLvl;
        }

        if (typeof attributes.book_xp === "string") {
            const cleaned = attributes.book_xp.replace(/\[object Object\]/g, '');
            attributes.book_xp = parseInt(cleaned, 10) || 0;
        } else {
        ...
        if (attributes.book_xp >= 10) {
            ...
            const finalLevel = initialLevel + levelsToAward;
            ...
            if (!fs.existsSync(bpFilePath)) {
                return { success: false, data: "Battle Pass data not found." };
            }
            ...
            Array.from({ length: levelsToAward }, (_, i) => initialLevel + i)
                .forEach(rewardTier => {
                    
                    if (bpData.freeRewards && bpData.freeRewards[rewardTier]) {
                        const freeReward = bpData.freeRewards[rewardTier];
                        
                        if (freeReward && Object.keys(freeReward).length > 0) {
                            Object.entries(freeReward).forEach(([templateId, quantity]) => {
                                
                                if (templateId.toLowerCase().includes("athenaseasonxpboost")) {
                                    attributes.season_match_boost = (attributes.season_match_boost || 0) + quantity;
                                } else if (templateId.toLowerCase().includes("athenaseasonfriendxpboost")) {
                                    attributes.season_friend_match_boost = (attributes.season_friend_match_boost || 0) + quantity;
                                } else if (templateId.toLowerCase().includes("mtxgiveaway") || templateId.toLowerCase().startsWith("currency:mtx")) {
                                    incrementCurrency(quantity);
                                } else if (templateId.toLowerCase().startsWith("homebasebanner")) {
                    ...
                    if (attributes.book_purchased === true && bpData.paidRewards && bpData.paidRewards[rewardTier]) {
                        const paidReward = bpData.paidRewards[rewardTier];
                        
                        if (paidReward && Object.keys(paidReward).length > 0) {
                            Object.entries(paidReward).forEach(([templateId, quantity]) => {
            ...
            attributes.book_xp -= levelsToAward * 10;
        ...
        await Profile.updateOne({ accountId: user.accountId }, { $set: findProfiles });
